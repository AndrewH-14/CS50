...
function Player:checkIfWon()
    if self.map:tileAt(self.x + self.width + 1, self.y).id == FLAG_TOP then
        GameState = 'won'
    elseif self.map:tileAt(self.x - 1, self.y).id == FLAG_TOP then
        GameState = 'won'
    elseif self.map:tileAt(self.x, self.y + self.height + 1).id == FLAG_TOP then
        GameState = 'won'
    end
end
